...
const CircleBorder = styled.div`
  border-radius: 50%;
  background-color: ${({ toggle }) => (toggle ? 'transparent' : 'var(--grey)')};
  width: 100px;
  height: 100px;
  color: var(--white);
  display: grid;
...
export default function Rocket() {
  ...
  const { scale, transform } = useSpring({
    from: { scale: 0, transform: 0 },
    to: async (next) => {
      await next({ scale: toggle ? 1 : 0 });
      await next({ transform: toggle ? -1 * media.height : 0 });
    },
    config: { ...config.molasses },
  });

  return (
    <CircleBorder onClick={() => setToggle(!toggle)} toggle={toggle}>
      <animated.div
        style={{
          transform: transform.to((value) => `translateY(${value}px)`),
          display: 'flex',
	...
	C236.141,345.385,211.862,335.448,211.862,335.448z"
          ...
          <ScrollText toggle={toggle}>SCROLL</ScrollText>
        </div>
      </animated.div>
    </CircleBorder>
  );
